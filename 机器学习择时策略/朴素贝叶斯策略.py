...
"""
    策略思想：选取一些常用技术指标作为训练样本特征数据，对于类别，如果
            之后22个工作日上涨幅度超过5%标记为1，否则标记为-1，采用朴素
            贝叶斯算法进行训练，预测结果为1且未持仓则买入，预测结果为-1
            且已持仓则卖出。

    特征因子选取：选取MACD指数平滑异同平均、MTM动力指标、BIAS乖离率、
            CCI顺势指标、KDJ随机指标、WR威廉指标、RSI相对强弱指标、
            ARBR人气意愿指标、OBV能量潮共9个技术指标。

    数据标准化：数据标准化方法有很多，本文采用高斯预处理方法，即每个特征
            因子减去它对应的均值再除以它的标准差((x-x.mean)/x.std)。
